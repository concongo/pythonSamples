...
#                   There is only ONE queue, and The order of the queue NEVER changes, and
...
#                   of a thread pool, with relation to running multiple processes at the same 
...
def queue_time(customers, n):
    ...
    while len(customers) >= 1:
        for i in range(0,n):
            if w[i]==tick and len(customers) >= 1:
                w[i]=customers[0]+tick
